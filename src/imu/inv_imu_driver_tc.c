...
int16_t accel_gyro_odr_values[] = {6400, 6400, 6400, 6400, 3200, 1600, 800, 400, 
								   200, 100, 50, 25, 12};
...
int inv_imu_init_tc(inv_imu_device_t *s)
{
	...
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_2, 2, (uint8_t *)&zero16);	// gyro_x_offuser
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_4, 2, (uint8_t *)&zero16);	// gyro_y_offuser
	status |= inv_imu_write_reg(s, IPREG_SYS1_REG_6, 2, (uint8_t *)&zero16);	// gyro_z_offuser	
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_12, 2, (uint8_t *)&zero16);	// accel_x_offuser
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_14, 2, (uint8_t *)&zero16);	// accel_y_offuser
	status |= inv_imu_write_reg(s, IPREG_SYS2_REG_16, 2, (uint8_t *)&zero16);	// accel_z_offuser
...
static uint8_t	calculate_lpf_cfg(int16_t N)
{
	...
	uint8_t tmp_lpf_cfg_range[] = {3, 5, 8, 15, 35, 75}; /*Changed 1st value to 3 in accordance with note on setting 0*/
	uint8_t i;
...
int inv_imu_get_tc_register_data(inv_imu_device_t *s, inv_imu_float_sensor_data_t *fl_data)
{
	...
	if (data.gyro_data[0] != INVALID_SENSOR_DATA ||
		data.gyro_data[1] != INVALID_SENSOR_DATA ||
...
static void read_gyr_off_values(inv_imu_device_t *s, int index, int16_t *gyr_off_vals)
{
	...
	inv_imu_read_reg(s, gyr_off_addrs[index], sizeof(gyr_off_temp),
					 (uint8_t *)gyr_off_temp);
	gyr_off_vals[0] = (gyr_off_temp[0] >> 2) | ((gyr_off_temp[1] & 0x7f) << 6);
	gyr_off_vals[1] = (gyr_off_temp[1] >> 7) | (gyr_off_temp[2] << 1) | 
	                  (gyr_off_temp[3] & 0x0f) << 9;
	gyr_off_vals[2] = (gyr_off_temp[3] >> 4) | (gyr_off_temp[4] << 4) |
					  (gyr_off_temp[5] & 0x01) << 12;
...
static void write_gyr_off_values(inv_imu_device_t *s, int index, int16_t *gyr_off_vals)
{
	...
	inv_imu_write_reg(s, gyr_off_addrs[index], sizeof(gyr_off_temp),
					(uint8_t *)gyr_off_temp);
...
static void read_acc_off_values(inv_imu_device_t *s, int index, int16_t *acc_off_vals)
{
	...
	inv_imu_read_reg(s, acc_off_addrs[index], sizeof(acc_off_temp),
						(uint8_t *)acc_off_temp);
...
static void write_acc_off_values(inv_imu_device_t *s, int index, int16_t *acc_off_vals)
{
	...
	inv_imu_write_reg(s, acc_off_addrs[index], sizeof(acc_off_temp),
					  (uint8_t *)acc_off_temp);
}

/*
 * Adjust the temperature compensation offset values.
 * The formula for calculating the new values is: 
 *                          offsetTCnew = offsetTC-gainTC*(offset/gain)
 *
 * Where:
 * offsetTC is the pre-correction value in:  
 *			gyro_{x,y,z}_mems_offtc{a,b}_{hfs,lfs}/2^11
 *			accel_{x,y,z}_mems_offtc{a,b}_{hfs32,hfs,lfs}/2^15
 * gainTC is the pre-zeroing value in:           
 *			gyro_{x,y,z}_gaintc{a,b}_{hfs,lfs}/2^17
 *			accel_{x,y,z}_gaintc{a,b}_{hfs32,hfs,lfs}/2^17
 * offset is the value stored in:       	           
 *			gyro_{x,y,z}_mems_off_{hfs,lfs}/2^7
...
static void adjust_tc_offset(inv_imu_device_t *s)
{
	...
	for (int i = 0; i < 3; i++) {
		...
		reg_value16 = (int16_t)reg_value12[0] | (int16_t)((reg_value12[1] & 0x0F) << 8); /* This is only a 12 bit value */
	...
	for (int i = 0; i < 3; i++) {
		gyr_offs_tc[i].a_hfs -= gyr_gain_tc[i].a_hfs * 
								(gyr_mems_off_tc[i].hfs / gyr_mems_gain_tc[i]);
		gyr_offs_tc[i].a_lfs -= gyr_gain_tc[i].a_lfs * 
								(gyr_mems_off_tc[i].lfs / gyr_mems_gain_tc[i]);
		gyr_offs_tc[i].b_hfs -= gyr_gain_tc[i].b_hfs * 
								(gyr_mems_off_tc[i].hfs / gyr_mems_gain_tc[i]);
		gyr_offs_tc[i].b_lfs -= gyr_gain_tc[i].b_lfs * 
								(gyr_mems_off_tc[i].lfs / gyr_mems_gain_tc[i]);	
		
		acc_offs_tc[i].a_hfs -= acc_gain_tc[i].a_hfs * 
								(acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
		acc_offs_tc[i].a_hfs32 -= acc_gain_tc[i].a_hfs32 * 
								  (acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
		acc_offs_tc[i].a_lfs -= acc_gain_tc[i].a_lfs * 
								(acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
		acc_offs_tc[i].b_hfs -= acc_gain_tc[i].b_hfs * 
								(acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
		acc_offs_tc[i].b_hfs32 -= acc_gain_tc[i].b_hfs32 * 
								  (acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
		acc_offs_tc[i].b_lfs -= acc_gain_tc[i].b_lfs * 
								(acc_mems_off_tc[i].hfs / acc_mems_gain_tc[i]);
	}
