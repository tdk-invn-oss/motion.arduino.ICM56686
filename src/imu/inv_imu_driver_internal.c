...
int inv_imu_get_sensor_data_length(inv_imu_device_t *s, inv_imu_sensor_reg_len_t *sensor_data_length)
{
	int         status = INV_IMU_OK;
	sreg_ctrl_internal_t sreg_ctrl;
...
int inv_imu_get_register_data_internal(inv_imu_device_t *s, inv_imu_sensor_data_internal_t *data)
{
	...
	if (sensor_data_length == INV_IMU_16_BIT_DATA) {
		...
		FORMAT_16_BITS_DATA(s->endianness_data, (uint8_t *)&data->sixteen_bits.accel_data[0],
		                    (uint16_t *)&data->sixteen_bits.accel_data[0]);
		FORMAT_16_BITS_DATA(s->endianness_data, (uint8_t *)&data->sixteen_bits.accel_data[1],
		                    (uint16_t *)&data->sixteen_bits.accel_data[1]);
		FORMAT_16_BITS_DATA(s->endianness_data, (uint8_t *)&data->sixteen_bits.accel_data[2],
		...
		/* Format accel data from sensor registers. */
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[0], (low_order_data[0] & 0x0F),
		                    &data->twenty_bits.accel_data[0]);
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[2], (low_order_data[1] & 0x0F),
		                    &data->twenty_bits.accel_data[1]);
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[4], (low_order_data[2] & 0x0F),
		                    &data->twenty_bits.accel_data[2]);
		/* Format gyro data from sensor registers. */
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[6], (low_order_data[0] >> 4),
		                    &data->twenty_bits.gyro_data[0]);
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[8], (low_order_data[1] >> 4),
		                    &data->twenty_bits.gyro_data[1]);
		FORMAT_20_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[10], (low_order_data[2] >> 4),
		                    &data->twenty_bits.gyro_data[2]);
