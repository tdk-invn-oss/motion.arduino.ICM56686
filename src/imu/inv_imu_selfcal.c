                        ...
                        inv_imu_selfcal_output_t *sc_output)
{
	...
	if (sc_params->gyro_en) {
		...
		if ((sc_params->gyro_method[0] == SELFCAL_STEP_RESPONSE) ||
			...
			rc |= run_selfcal(s, sc_params, SELFCAL_STEP_RESPONSE, sc_output, &sc_internal_output);
			if (sc_output->gyro_status == INV_IMU_SC_STATUS_SUCCESS) {
				rc |= calc_gyro_step_response_usergain(s, sc_params, &sc_internal_output,
		...
		if ((sc_params->gyro_method[0] == SELFCAL_SC2V_ADC) ||
			...
			rc |= run_selfcal(s, sc_params, SELFCAL_SC2V_ADC, sc_output, &sc_internal_output);
			if (sc_output->gyro_status == INV_IMU_SC_STATUS_SUCCESS)
	...
	if (sc_params->accel_en) {
		...
		if (sc_output->accel_status != INV_IMU_SC_STATUS_SUCCESS)
			return INV_IMU_ERROR;
		rc |= calc_accel_usergain(s, sc_params, &sc_internal_output, sc_output->accel_gains);
	}
                                  ...
                                  const selfcal_gyro_method_t         gyro_method)
{
	...
	tmp_stc_params &= ~(SELFTESTCAL_INIT_EN_MASK | SELFTEST_ACCEL_EN_MASK | SELFTEST_GYRO_EN_MASK |
	...
	rc |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_STC_PATCH_EN, (uint8_t *)&(sc_params->patch_settings));
                              ...
                              inv_imu_selfcal_output_internal_t *sc_output_i)
{
	...
	if (sc_params->accel_en) {
		...
		INV_IMU_SC_STATUS_SUCCESS : INV_IMU_SC_STATUS_FAIL;
