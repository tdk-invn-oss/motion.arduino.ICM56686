...
/* 
 * Contains registers value that will be 
 * modified during the procedure 
 */
typedef struct {
...
STATIC_INLINE int run_edmp_ondemand(inv_serif_auth_t *s)
{
     ...
     * registers starting with register STATUS_MASK_PIN_0_7_IPREG_TOP1 for interrupt0.
     */
...
STATIC_INLINE int save_settings(inv_serif_auth_t *s, device_settings_t *settings)
{
	...
	status |= read_mreg(s, STATUS_MASK_PIN_16_23_IPREG_TOP1, 1, &settings->status_mask_pin_16_23);
...
STATIC_INLINE int recover_settings(inv_serif_auth_t *s, device_settings_t *settings)
{
	...
	status |= write_mreg(s, STATUS_MASK_PIN_8_15_IPREG_TOP1, 1, &settings->status_mask_pin_8_15);
	status |= write_mreg(s, STATUS_MASK_PIN_16_23_IPREG_TOP1, 1, &settings->status_mask_pin_16_23);
